"""
NOC client for TPAP (bundled in kasa_tpap_protocols).

- ACCESS_KEY and SECRET_KEY are hard-coded protocol constants extracted from the APK.
- NOCClient.apply(username, password) -> TpapNOCData
- The client automatically loads the bundled root_ca.pem from the package and
  writes it to a temporary file; that file path is then passed to requests.verify
  for TLS verification. If the bundled PEM cannot be loaded, requests will use
  system CAs (verify=True) as a fallback and a warning is logged.
"""
...
def _ensure_ca_file() -> str | None:
    ...
    Write the root CA to a temporary file. If fetching fails, returns
    None so callers can fall back to system CAs.
    """
    ...
    with _CA_FILE_PATH_LOCK:
        ...
        try:
            ...
            if not data:
                _LOGGER.debug("kasa_tpap_protocols: could not fetch root CA from host")
                _CA_FILE_PATH = None
                return None
            ...
            def _cleanup() -> None:
                try:
                    ...
                    _LOGGER.debug("kasa_tpap_protocols: cleanup unlink failed: %s", exc)

            atexit.register(_cleanup)
            _LOGGER.debug(
                "kasa_tpap_protocols: fetched root CA and wrote to %s", _CA_FILE_PATH
            )
            return _CA_FILE_PATH
        except Exception as exc:
            _LOGGER.debug("kasa_tpap_protocols: unable to fetch/write root CA: %s", exc)
            _CA_FILE_PATH = None
            return None
...
def _fetch_root_ca(hostname: str) -> bytes | None:
    ...
    Return the root CA as PEM bytes or None on failure.
    """
    try:
        # get server leaf cert PEM
        leaf_pem = ssl.get_server_certificate((hostname, 443))
        try:
            cert = crypto_x509.load_pem_x509_certificate(leaf_pem.encode())
        except Exception:
            _LOGGER.debug("kasa_tpap_protocols: failed to parse leaf cert PEM")
            return None
        ...
        while True:
            ...
            # try to get CA Issuers URLs from AIA extension
            urls = []
            try:
            ...
            if not urls:
                # no AIA links — give up and return the current cert as a
                # best-effort root
                return cert.public_bytes(serialization.Encoding.PEM)
            ...
            for url in urls:
                try:
                    ...
                    _LOGGER.debug(
                        "kasa_tpap_protocols: failed to fetch AIA url %s: %s", url, exc
                    )
                    continue
        ...
        _LOGGER.debug("kasa_tpap_protocols: _fetch_root_ca failed: %s", exc)
        return None
...
class NOCClient:
    ...
    - No constructor args (matches usage in python-kasa).
    - apply raises RuntimeError on failures (so python-kasa error handling will see it).
        - HTTPS requests use bundled CA file when available; otherwise
          system CAs are used.
    """
    ...
    def _verify_arg(self) -> object:
        """
        Return value suitable for requests.verify.

        Return path to bundled CA file if available, otherwise True.
        """
    ...
    def _split_chain(chain_pem: str) -> tuple[str, str]:
        ...
        # Strip any leading whitespace/newlines that may occur before the
        # next certificate begins in the chain to avoid a blank line at the
        # start of the root PEM file when saved.
    ...
    def apply(self, username: str, password: str) -> TpapNOCData:
        ...
        All HTTPS calls verify the server using the bundled root CA (if present).
        """
